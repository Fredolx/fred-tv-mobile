...
pub(crate) async fn get_xtream(mut source: Source, wipe: bool) -> Result<()> {
    ...
    live.and_then(|live| process_xtream(&tx, live, live_cats?, &source, MediaType::Livestream))
        .unwrap_or_else(|e| {
            log::log(format!("{:?}", e.context("Failed to process live")));
            fail_count += 1;
        });
    vods.and_then(|vods: Vec<XtreamStream>| {
        process_xtream(&tx, vods, vods_cats?, &source, MediaType::Movie)
    })
    .unwrap_or_else(|e| {
    ...
    series
        .and_then(|series: Vec<XtreamStream>| {
            process_xtream(&tx, series, series_cats?, &source, MediaType::Serie)
        })
        .unwrap_or_else(|e| {
...
pub(crate) async fn get_episodes(channel: Channel) -> Result<()> {
